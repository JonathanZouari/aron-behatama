...
uuid → text, jsonb → text, timestamptz → text, numeric(p,s) → text,
boolean נשאר (SQLite מקבל 0/1), numeric → text (דיוק כספי), ומשפטי RLS מוסרים.
"""
...
def to_sqlite(sql: str) -> str:
    ...
    out = re.sub(r"\bnumeric\(\d+,\s*\d+\)", "text", out, flags=re.IGNORECASE)  # כסף כטקסט: NUMERIC affinity ממיר ל-REAL ומאבד דיוק
